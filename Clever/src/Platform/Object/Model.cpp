...
namespace Clever
{
	...
	Model::Model(const std::string& objFilePath, CompiledShader shader)
		: m_Shader(shader)
	{
		...
		const aiScene* pScene = Importer.ReadFile(objFilePath.c_str(),
			aiProcess_Triangulate | aiProcess_GenSmoothNormals | 
			aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices);

		if (pScene)
			Ret = InitFromScene(pScene, objFilePath);
		else
			CV_CORE_ERROR("Error parsing {0} : {1}", objFilePath.c_str(), Importer.GetErrorString());
		...
		glGenBuffers(1, &transformationsID);
	...
	void Model::recalculateInstancedList(std::vector<glm::mat4> transformData)
	{
		m_VertexArray->Bind();
		glBindBuffer(GL_ARRAY_BUFFER, transformationsID);
		glBufferData(GL_ARRAY_BUFFER, transformData.size() * sizeof(glm::mat4), &transformData.front(), GL_STATIC_DRAW);
	...
	void Model::UpdatedColors(std::vector<glm::vec3> colorData)
	{
		m_VertexArray->Bind();
		glBindBuffer(GL_ARRAY_BUFFER, colorsID);
		glBufferData(GL_ARRAY_BUFFER, colorData.size() * sizeof(glm::vec3), &colorData.front(), GL_STATIC_DRAW);
		glVertexAttribPointer(6, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), (void*)(sizeof(float) * 0));
	...
	void Model::loadModel(std::string const& path)
	{
		...
		{
			CV_CORE_ERROR("ERROR::ASSIMP:: {0}", importer.GetErrorString());
			CV_CORE_ASSERT("Failed to load", true);
			return;
		}
